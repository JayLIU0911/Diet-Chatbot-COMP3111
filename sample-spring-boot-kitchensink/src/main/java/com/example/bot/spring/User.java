...
public class User {
    ...
    public User (ArrayList<String> list){
        ...
        try {
            ...
            if (userAdapter.insert(this)){
            
                return;
            }
        }catch (Exception e){
            log.info("insert user failure", e.toString());
        }
    }
    ...
    public File generateWeeklySummary () {
       ...
       String[][] pass7Record = new String[userAdapter.searchRecord(id).size()][5];// Record[date][date, weight, calories, sodium, fat]
        ...
        for (int i=0;i<7;i++){
            ...
            for(int j=1; j<4; j++){
                pass7summary[i][j] = Float.parseFloat(Record.get(i).get(j+1));
            }
        }
        
        String[] date = new String[7];
        for (int i=0; i<7; i++){
            date[i] = new String(pass7Record[i][0]);
        }
