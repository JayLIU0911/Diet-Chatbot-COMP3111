...
//            if(rs.next()){
	...
	public String generateSummary(String id) {
        ...
        String output = "So far, you have consumed: \n" + overall[0] + " calories, \n" + overall[1] + " sodium and \n" + overall[2] + "fatty acid ";
    ...
    public String generateWeeklySummary (String id) {
        ...
        float[] sum= new float[4]; // calories, sodium, fat
        
        for (int i=0;i<date_for_summary;i++){
            
            for (int j=0; j<4; j++){
                sum[j] += Float.parseFloat(Record.get(number_of_date-i+date_for_summary-1).get(j+1);
            };
            
                                           
        };
        
        String output = "In the past " + date_for_summary + " days, your average weight and daily energy intake are: " + sum[0]/date_for_summary +
        "kg and "+ sum[1]/date_for_summary + "kcals respectively. \n" + "your detailed weight and energy intake daily changes are as follow \n";
                                           
                                           
                                           
                                           
        for ( int i=0; i<date_for_summary; i++){
            output = output + Record.get(number_of_date-i+date_for_summary-1).get(0) + ": " + Record.get(number_of_date-i+date_for_summary-1).get(1) + "kg, " + Record.get(number_of_date-i+date_for_summary-1).get(2) + "kcals \n";
        }
        return output;
